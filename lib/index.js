...
function apply(ctx) {
  ...
  function resolveTurn(rootId, exec) {
    ...
    // 3) 仍无：递增到下一轮，避免复用同一轮导致 header 错标
    const filesTmp = sessions.get(rootId)
    const m = filesTmp ? maxTurn(filesTmp) : 0
    return m ? m + 1 : 1
  }
