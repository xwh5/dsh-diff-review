...
// 正常路径有 session/event 订阅（各事件自带 data.turn，精确），此窗口只是最后防线。
...
function apply(ctx) {
  ...
  // and covers resumed sessions whose restored events never dispatch.
  ...
  function currentTurnOf(rootId) {
    ...
    try {
      ...
      if (events && Array.isArray(events) && events.length > cur.scanSeq) {
        ...
        // 宿主事件无 turn/start 类型：每个事件自带 data.turn，取最近的即可
        for (let i = events.length - 1; i >= from; i--) {
          const e = events[i]
          const t = e && e.data && e.data.turn
          if (typeof t === 'number' && t > 0) { cur.turn = t; break }
        }
      }
    } catch (e) {}
  ...
  // ── 真实 turn 订阅（首选信号）────────────────────────────────────────
  // session.events 数组在宿主上不可用，且宿主事件中没有 turn/start|turn/end
  // 类型（事件日志证实）。但每个 session/event（assistant/chunk、tool/call、
  // user/message…）都自带 data.turn = 当前轮号 —— 直接读它维护 liveTurns，
  // 使同一轮内的多次写文件归入同一 turn（修复“最新一轮只显示最新一个文件”）。
  // user/message 视为新一轮：data.turn 缺省时置 null，等下一条带 turn 的事件。
  ...
  function resolveTurn(rootId, exec) {
    ...
    // 3) 扫尾部事件（宿主事件自带 data.turn，取最近一个）
