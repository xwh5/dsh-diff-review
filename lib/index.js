...
function loadSessions(sessions, file) {
  ...
  try {
    ...
    for (const [sid, s] of Object.entries(data.sessions)) {
      ...
      // 迁移：历史 bug 导致整会话全为 turn 0（latestTurn=0 时前端视为空），统一抬为 1 使“最新一轮”可见
      let allZero = true
      let hasAny = false
      for (const rec of files.values()) {
        for (const op of rec.ops) {
          hasAny = true
          if (typeof op.turn === 'number' && op.turn !== 0) { allZero = false; break }
        }
        if (!allZero) break
      }
      if (hasAny && allZero) {
...
function apply(ctx) {
  ...
  ctx.on('tools/result', (exec, result) => {
    try {
      ...
      const at = Date.now()
      let turn = currentTurnOf(rootId)
      // Fallback: 若事件扫描拿不到 turn（例如 session.events 未就绪或类型变更），直接取 exec 自带的 turn/step
      if (!turn) {
        const cand = exec.turn ?? exec.data?.turn ?? exec.step ?? exec.data?.step ?? null
        if (typeof cand === 'number' && Number.isFinite(cand) && cand > 0) turn = cand
        else if (exec.data && typeof exec.data.turn === 'number' && exec.data.turn > 0) turn = exec.data.turn
        // 兜底：若仍为 0，尝试从 agent.session 的最后一次 turn/start 直接读
        if (!turn) {
          try {
            const ent = ctx.agents && ctx.agents.store && ctx.agents.store.get(rootId)
            const evs = ent && ent.agent && ent.agent.session && ent.agent.session.events
            if (Array.isArray(evs)) {
              for (let i = evs.length - 1; i >= 0; i--) {
                const e = evs[i]
                if (e && e.type === 'turn/start' && e.data && typeof e.data.turn === 'number' && e.data.turn > 0) { turn = e.data.turn; break }
                if (e && e.type === 'turn/end') break
              }
            }
          } catch {}
        }
        // 最终仍为 0 时，记为 1 保证最新一轮有数据（避免 0 轮被前端当空态）
        if (!turn) {
          // 若该会话已有记录，复用最近一次非 0 turn；否则用 1
          let reuse = 0
          const filesTmp = sessions.get(rootId)
          if (filesTmp) {
            for (const r of filesTmp.values()) for (const op of r.ops) if (typeof op.turn === 'number' && op.turn > reuse) reuse = op.turn
          }
          turn = reuse || 1
        }
      }
  ...
  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: '/diff-review/debug',
    handler: (req, res) => {
      const session = queryParam(req, 'session')
      const cur = turnCursor.get(session)
      const entry = ctx.agents && ctx.agents.store && ctx.agents.store.get(session)
      const sess = entry && entry.agent && entry.agent.session
      const evs = sess && sess.events
      const rootId = resolveRootId(session)
      const curTurn = currentTurnOf(session)
      sendJson(res, 200, {
        session,
        rootId,
        curTurn,
        cursor: cur ? { turn: cur.turn, scanSeq: cur.scanSeq } : null,
        eventsLen: Array.isArray(evs) ? evs.length : null,
        lastEvents: Array.isArray(evs) ? evs.slice(-10).map(e => ({ type: e.type, data: e.data, seq: e.seq })) : null,
        storeKeys: ctx.agents && ctx.agents.store ? Array.from(ctx.agents.store.keys()).slice(0,5) : null,
        hasEntry: !!entry
      })
    }
  }), 'diff-review: debug route')
